...
varname="type";
es.particle_properties.setType(0,numpy.random.randint(low = 0, high = 100))
...
# Test particle data interface using TCL for comparison
print "\nTesting particle data..."
VarId=0;
varname="pos";
es.part[1].pos = numpy.random.random(3)
tcl_val = es._espressoHandle.Tcl_Eval("part " + str(1) + " print " + varname).split()
py_val = es.part[1].pos
for i in range(3):
    if ( ( numpy.fabs(float(tcl_val[i])) - numpy.fabs(py_val[i]) ) > 1e-10 ):
        raise ValueError(varname + " FAILED\n" + "Tcl".ljust(10) + str(tcl_val) + "\n" +  "python".ljust(10) + str(py_val) + "\n");
print (str(VarId)+" "+varname).ljust(20), "OK";
...
varname="type";
es.part[1].type = numpy.random.randint(low = 0, high = 100)
py_val = es.part[1].type
tcl_val = int(es._espressoHandle.Tcl_Eval("part " + str(1) + " print " + varname))
if py_val != tcl_val:
    raise ValueError(varname + " FAILED\n" + "Tcl".ljust(10) + str(tcl_val) + "\n" +  "python".ljust(10) + str(py_val) + "\n");
print (str(VarId)+" "+varname).ljust(20), "OK";
VarId=VarId+1;
...
try:
    varname="q"
    es.part[1].q = numpy.random.random(1)
    py_val = es.part[1].q
    tcl_val = es._espressoHandle.Tcl_Eval("part " + str(1) + " print " + str(varname))
    if ( ( numpy.fabs(float(tcl_val)) - numpy.fabs(py_val) ) > 1e-10 ):
        raise ValueError(varname + " FAILED\n" + "Tcl".ljust(10) + str(tcl_val) + "\n" +  "python".ljust(10) + str(py_val) + "\n");
    print (str(VarId)+" "+varname).ljust(20), "OK";
except ValueError as detail:
...
try:
    varname="mass"
    es.part[1].mass = numpy.random.random(1)
    py_val = es.part[1].mass
    tcl_val = es._espressoHandle.Tcl_Eval("part " + str(1) + " print " + str(varname))
    if ( ( numpy.fabs(float(tcl_val)) - numpy.fabs(py_val) ) > 1e-10 ):
        raise ValueError(varname + " FAILED\n" + "Tcl".ljust(10) + str(tcl_val) + "\n" +  "python".ljust(10) + str(py_val) + "\n");
    print (str(VarId)+" "+varname).ljust(20), "OK";
